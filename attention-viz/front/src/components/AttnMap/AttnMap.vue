    ...
    <div class="viewHead" id="attn-map-view">
        ...
        <div class="checkbox-contain">
            ...
            <div class="half" v-show="model == 'gpt-2'">
             ...
             <!-- LLaVA Controls -->
            <div :class="{ half: true }" v-show="model == 'LLaVA-1.5'">
                <p class="label">Hide<a-tooltip placement="leftTop">
                        <template #title>
                            <span>filter out attention to special tokens</span>
                        </template>
                        <font-awesome-icon icon="info" class="info-icon" />
                    </a-tooltip></p>
                <a-checkbox v-model:checked="hideFirst" @click="hideTokens('first')">first token</a-checkbox>
            </div>
        </div>
        <Transition>
