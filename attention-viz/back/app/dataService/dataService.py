...
class DataService(object):
    def __init__(self):
        ...
        # LLaVA-1.5
        self.matrix_data_llava_15 = read_matrix_data("llava_1.5")
        self.attention_data_llava_15 = read_attention_data("llava_1.5")
        self.agg_att_data_llava_15 = read_agg_attn_data("llava_1.5")
        self.token_data_llava_15 = read_token_data("llava_1.5")
    ...
    def get_matrix_data(self, model):
        ...
        elif model == "LLaVA-1.5":
            return self.matrix_data_llava_15
        elif model == "gpt":
    ...
    def get_token_data(self, model):
        if model == "bert":
            ...
            return self.token_data_llava_15
        elif model == "gpt":
    ...
    def get_attention_by_token(self, token, model):
        ...
        if model == "bert":
            ...
            all_token_info = self.token_data_llava_15['tokens'][index]
            offset = len(self.token_data_llava_15['tokens']) / 2
        else:
        ...
        if model == "bert":
            ...
            attn_data = self.attention_data_llava_15
            agg_attns = self.agg_att_data_llava_15['{}_{}'.format(
                layer, head)][:num_tokens]
        else:
        ...
        agg_attn = [] if model not in ["bert", "gpt-2"] else normalize_attn([t['attention'][:num_tokens]
                                                                             for t in agg_attns])
        if (model == "LLaVA-1.5"):
            agg_attn = normalize_attn([t['attention'][:num_tokens] for t in agg_attns])

        norms = [] if model != "gpt-2" else [t['value_norm'] for t in attns]
        if (model == "LLaVA-1.5"):
            norms = [t['value_norm'] for t in attns]
        agg_norms = [] if model != "gpt-2" else [t['value_norm'] for t in agg_attns]
        if (model == "LLaVA-1.5"):
            agg_norms = [t['value_norm'] for t in agg_attns]
        if model in ["vit-32", "vit-nat", "vit-syn"]:
